...
COMPRESS ?= lz4t
$(eval $(call validate-option,COMPRESS,mio0 yay0 lz4 lz4t lz4u gzip rnc1 rnc2 uncomp))
ifeq ($(COMPRESS),gzip)
  DEFINES += GZIP=1
...
LZ3TPACK              := $(TOOLS_DIR)/lz3tpack
LZ3UPACK              := $(TOOLS_DIR)/lz3upack
...
include compression/lz4urules.mk
else ifeq ($(COMPRESS),uncomp)
