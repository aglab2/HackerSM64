...
static int do_compress(const char *pszInFilename, const char *pszOutFilename, const char *pszDictionaryFilename, const unsigned int nOptions, const unsigned int nMaxWindowSize) {
   ...
   uint32_t decompressHeader[2] = { __builtin_bswap32(stats.safe_dist), __builtin_bswap32(nOriginalSize) }; // Modified for SM64 so the game knows how much to allocate for decompression and work buffer
