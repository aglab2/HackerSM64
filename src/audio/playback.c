...
#define POP(item)                                                                                      \
    ...
    for (i = 0; i < gMaxSimultaneousNotes; i++) {
        ...
        if (note->priority != NOTE_PRIORITY_DISABLED) {
            ...
            velocity = velocity * scale * scale;
            if (((note->bankId == 0x08) || (note->bankId == 0x0A)) && ((gLuigiModel) || (gLuigiOverride))) { // Reonu: Change pitch shift for Luigi. gLuigiModel and gLuigiOverride are never 1 outside of my level so dw
