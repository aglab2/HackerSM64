...
void spiders_arrow_init()
{
    if (gCurrActNum != 5)
        cur_obj_hide();
}

void spiders_arrow_loop()
{
    f32 d;
    ...
    if (!red)
    {
        red = cur_obj_find_nearest_object_with_behavior(bhvHiddenRedCoinStar, &d);
    }
    if (!red)
    {
        red = cur_obj_find_object_with_behavior_and_bparam1(bhvStar, 4);
    }
