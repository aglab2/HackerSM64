...
void mode_8_directions_camera(struct Camera *c) {
 ...
 // extra functionality
    ...
    lakitu_zoom(!gTriggerHell ? 400.f : 10000.f, 0x900);
...
void update_camera(struct Camera *c) {
    ...
    if (c->cutscene == 0 &&
        ...
        if ((cam_select_alt_mode(0) == CAM_SELECTION_MARIO) && (gCurrLevelNum != LEVEL_CASTLE_GROUNDS) && (gPlayer1Controller->buttonPressed & R_TRIG)) {
    ...
    if (!gPuppyCam.enabled || c->cutscene != 0 || gCurrentArea->camera->mode == CAMERA_MODE_INSIDE_CANNON) {
        ...
        if (gCurrLevelNum != LEVEL_CASTLE) {
            ...
            if ((c->cutscene == 0 &&
                (gPlayer1Controller->buttonDown & R_TRIG) && (gCurrLevelNum != LEVEL_CASTLE_GROUNDS) && cam_select_alt_mode(0) == CAM_SELECTION_FIXED)
