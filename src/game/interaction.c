...
u32 interact_star_or_key(struct MarioState *m, UNUSED u32 interactType, struct Object *obj) {
 ...
 #endif
#else // !NON_STOP_STARS
    u32 noExit = gCurrLevelNum == LEVEL_BITDW || gCurrLevelNum == LEVEL_BITFS || gCurrLevelNum == LEVEL_BITS || (obj->oInteractionSubtype & INT_SUBTYPE_NO_EXIT) != 0;
...
void pss_end_slide(struct MarioState *m) {
    ...
    if (sPssSlideStarted) {
        ...
        if (slideTime < 35*30) {
            ...
            spawn_default_star(5377.0f, -2050.0f, 7890.0f);
        }
