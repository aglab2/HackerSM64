#include <ultra64.h>
#include "sm64.h"
#include "behavior_data.h"
#include "model_ids.h"
#include "seq_ids.h"
#include "dialog_ids.h"
#include "segment_symbols.h"
#include "level_commands.h"

#include "game/level_update.h"

#include "levels/scripts.h"

#include "actors/common1.h"

/* Fast64 begin persistent block [includes] */
/* Fast64 end persistent block [includes] */

#include "make_const_nonconst.h"
#include "levels/castle_grounds/header.h"

/* Fast64 begin persistent block [scripts] */
/* Fast64 end persistent block [scripts] */

const LevelScript level_castle_grounds_entry[] = {
	INIT_LEVEL(),
	LOAD_YAY0(0x07, _castle_grounds_segment_7SegmentRomStart, _castle_grounds_segment_7SegmentRomEnd), 
	LOAD_YAY0(0x0A, _water_skybox_yay0SegmentRomStart, _water_skybox_yay0SegmentRomEnd), 
	LOAD_YAY0_TEXTURE(0x09, _outside_yay0SegmentRomStart, _outside_yay0SegmentRomEnd), 
	LOAD_YAY0(0x05, _group10_yay0SegmentRomStart, _group10_yay0SegmentRomEnd), 
	LOAD_RAW(0x0C, _group10_geoSegmentRomStart, _group10_geoSegmentRomEnd), 
	LOAD_YAY0(0x06, _group15_yay0SegmentRomStart, _group15_yay0SegmentRomEnd), 
	LOAD_RAW(0x0D, _group15_geoSegmentRomStart, _group15_geoSegmentRomEnd), 
	LOAD_YAY0(0x08, _common0_yay0SegmentRomStart, _common0_yay0SegmentRomEnd), 
	LOAD_RAW(0x0F, _common0_geoSegmentRomStart, _common0_geoSegmentRomEnd), 
	LOAD_MIO0(0x7, _castle_grounds_segment_7SegmentRomStart, _castle_grounds_segment_7SegmentRomEnd), 
	LOAD_MIO0(0xa, _water_skybox_mio0SegmentRomStart, _water_skybox_mio0SegmentRomEnd), 
	ALLOC_LEVEL_POOL(),
	MARIO(MODEL_MARIO, 0x00000001, bhvMario), 
	JUMP_LINK(script_func_global_1), 
	JUMP_LINK(script_func_global_11), 
	JUMP_LINK(script_func_global_16), 
	LOAD_MODEL_FROM_GEO(MODEL_THI_WARP_PIPE, warp_pipe_geo), 
	LOAD_MODEL_FROM_GEO(MODEL_LEVEL_GEOMETRY_03, castle_grounds_geo_0006F4), 
	LOAD_MODEL_FROM_GEO(MODEL_CASTLE_GROUNDS_BUBBLY_TREE, bubbly_tree_geo), 
	LOAD_MODEL_FROM_GEO(MODEL_CASTLE_GROUNDS_WARP_PIPE, warp_pipe_geo), 
	LOAD_MODEL_FROM_GEO(MODEL_CASTLE_GROUNDS_CASTLE_DOOR, castle_door_geo), 
	LOAD_MODEL_FROM_GEO(MODEL_CASTLE_GROUNDS_METAL_DOOR, metal_door_geo), 
	LOAD_MODEL_FROM_GEO(MODEL_CASTLE_GROUNDS_VCUTM_GRILL, castle_grounds_geo_00070C), 
	LOAD_MODEL_FROM_GEO(MODEL_CASTLE_GROUNDS_FLAG, castle_grounds_geo_000660), 
	LOAD_MODEL_FROM_GEO(MODEL_CASTLE_GROUNDS_CANNON_GRILL, castle_grounds_geo_000724), 
	LOAD_MODEL_FROM_GEO(MODEL_OW_BRODUTE, ow_part_brodute), 
	LOAD_MODEL_FROM_GEO(MODEL_OW_ARTHUR, ow_part_arthur), 
	LOAD_MODEL_FROM_GEO(MODEL_OW_GAEL, ow_part_gael), 
	LOAD_MODEL_FROM_GEO(MODEL_OW_SIDE, ow_part_side), 
	LOAD_MODEL_FROM_GEO(MODEL_OW_SCUT, ow_part_scut), 
	LOAD_MODEL_FROM_GEO(MODEL_OW_BOWSER, ow_part_bowser), 
	LOAD_MODEL_FROM_GEO(MODEL_OW_DNVIC_CLOSE, ow_dnvic_close), 
	LOAD_MODEL_FROM_GEO(MODEL_CASTLE_KEY_DOOR, key_door_geo), 

	/* Fast64 begin persistent block [level commands] */
	/* Fast64 end persistent block [level commands] */

	AREA(1, castle_grounds_area_1),
		WARP_NODE(0xf0, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0xf1, LEVEL_CASTLE_GROUNDS, 0x01, 0xf1, WARP_NO_CHECKPOINT),
		WARP_NODE(0x21, LEVEL_AB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x31, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x41, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x29, LEVEL_DF, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x39, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x49, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x23, LEVEL_MF, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x33, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x43, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x25, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x35, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x45, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x55, LEVEL_BOB, 0x01, 0x0B, WARP_NO_CHECKPOINT),
		WARP_NODE(0x32, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x42, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x22, LEVEL_MTC, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x48, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x38, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x28, LEVEL_PSS, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x24, LEVEL_HF, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x34, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x44, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x27, LEVEL_VCM, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x37, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x47, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x50, LEVEL_BDF, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x60, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x70, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x51, LEVEL_TOTWC, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x61, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x71, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x52, LEVEL_COTMC, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x62, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x72, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x53, LEVEL_WMOTR, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x63, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x73, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x54, LEVEL_VCUTM, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x64, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x74, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x65, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x75, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x56, LEVEL_SA, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x66, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x76, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x57, LEVEL_CRASH, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x0A, LEVEL_BOB, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		WARP_NODE(0x80, LEVEL_CASTLE, 0x01, 0x0A, WARP_NO_CHECKPOINT),
		OBJECT(MODEL_NONE, 168, 872, -5037, 0, 180, 0, (0x41 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, 166, 872, -5026, 0, 179, 0, (0x31 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_CASTLE_KEY_DOOR, -2311, 3226, 1474, 0, 0, 0, 0x00000000, bhvDoor),
		OBJECT(MODEL_CASTLE_KEY_DOOR, -2157, 3226, 1474, 0, -180, 0, 0x00000000, bhvDoor),
		OBJECT(MODEL_NONE, 74, 399, -6458, 0, 0, 0, (20 << 24) | (0x21 << 16), bhvWarp),
		OBJECT(MODEL_THI_WARP_PIPE, 2497, -1057, -2695, 0, 0, 0, (0x29 << 16), bhvWarpPipe),
		OBJECT(MODEL_THI_WARP_PIPE, 3755, 440, 188, 0, 0, 0, (0x28 << 16) | (1 << 8), bhvWarpPipe),
		OBJECT(MODEL_THI_WARP_PIPE, 1193, -1108, 4740, 0, 0, 0, (0x24 << 16) | (3 << 8), bhvWarpPipe),
		OBJECT(MODEL_THI_WARP_PIPE, 2574, 1013, -2684, 0, 0, 0, (0x27 << 16), bhvWarpPipe),
		OBJECT(MODEL_THI_WARP_PIPE, -2262, 3226, -34, 0, 0, 0, (0x50 << 16), bhvWarpPipe),
		OBJECT(MODEL_NONE, 13, -621, -36, 0, 0, 0, 0x00000000, bhvOWCtl),
		OBJECT(MODEL_THI_WARP_PIPE, -2201, -967, -1190, 0, 0, 0, (0x54 << 16) | (2 << 8), bhvWarpPipe),
		OBJECT(MODEL_NONE, -1756, -732, -2769, 0, -17, 0, (0x23 << 16) | (2 << 8), bhvWarp),
		OBJECT(MODEL_NONE, -1647, -732, -3060, 0, -17, 0, (0x23 << 16) | (2 << 8), bhvWarp),
		OBJECT(MODEL_NONE, -1710, -732, -2905, 0, -17, 0, (0x23 << 16) | (2 << 8), bhvWarp),
		OBJECT(MODEL_NONE, -1466, -1649, 4256, 0, -17, 0, (20 << 24) | (0x53 << 16) | (3 << 8), bhvWarp),
		OBJECT(MODEL_STICKY_PLAT, 3450, -588, 4372, 0, -180, 0, (1 << 16) | (12), bhvStickyPlat),
		OBJECT(MODEL_WOODEN_SIGNPOST, 3529, -1104, 5019, 0, -90, 0, (46 << 24) | (46 << 16) | (5 << 8), bhvMessagePanel),
		OBJECT(MODEL_CASTLE_GROUNDS_METAL_DOOR, 3718, -1104, 4659, 0, -90, 0, (0x25 << 16) | (5 << 8), bhvDoorWarp),
		OBJECT(MODEL_CASTLE_GROUNDS_METAL_DOOR, 3720, -1104, 4811, 0, 90, 0, (0x55 << 16) | (5 << 8), bhvDoorWarp),
		OBJECT(MODEL_STICKY_PLAT, 3450, -169, 5121, 0, 0, 0, (1 << 16) | (12), bhvStickyPlat),
		OBJECT(MODEL_STICKY_PLAT, 3450, 314, 4372, 0, -180, 0, (1 << 16) | (12), bhvStickyPlat),
		OBJECT(MODEL_NONE, 3267, -1203, 176, 0, -90, 0, (10 << 24) | (0x22 << 16) | (1 << 8), bhvWarp),
		OBJECT(MODEL_TOAD, 4137, 502, 5191, 0, -180, 0, (82 << 24) | (82 << 16) | (5 << 8), bhvToadMessage),
		OBJECT(MODEL_NONE, 1656, -206, -1819, 0, 120, 0, (0x49 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, 3396, -599, 4767, 0, 90, 0, (0x45 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, 2657, -702, 129, 0, 90, 0, (0x42 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, 3175, 1070, 189, 0, 89, 0, (0x48 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, 786, -759, 3655, 0, 9, 0, (0x44 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, 1957, 1364, -2763, 0, 84, 0, (0x47 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -2251, 3610, 1157, 0, 180, 0, (0x70 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -3476, -670, 3376, 0, 24, 0, (0x71 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -3538, -721, 2074, 0, 157, 0, (0x72 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -537, -1026, 4145, 0, -80, 0, (0x73 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -1759, -455, -1470, 0, -64, 0, (0x74 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -4506, -663, 2790, 0, -91, 0, (0x75 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -2682, -1194, 2563, 0, 96, 0, (0x76 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, 1659, -206, -1808, 0, 120, 0, (0x39 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, 3399, -599, 4769, 0, 90, 0, (0x35 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, 2660, -702, 117, 0, 87, 0, (0x32 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, 3177, 1070, 177, 0, 90, 0, (0x38 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, 788, -759, 3644, 0, 9, 0, (0x34 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, 1959, 1364, -2774, 0, 80, 0, (0x37 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -2249, 3610, 1146, 0, 180, 0, (0x60 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -3477, -670, 3364, 0, 25, 0, (0x61 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -3537, -721, 2062, 0, 153, 0, (0x62 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -526, -1026, 4147, 0, -79, 0, (0x63 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -1761, -455, -1482, 0, -64, 0, (0x64 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -4496, -663, 2795, 0, -91, 0, (0x65 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -2693, -1194 + 100, 2559, 0, 96, 0, (0x66 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -1224, -445, -2648, 0, -115, 0, (0x43 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -1217, -445, -2658, 0, -119, 0, (0x33 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, -24, -315, -37, 0, -180, 0, (0xf1 << 16), bhvDeathWarp),
		OBJECT(MODEL_NONE, -14, -298, -16, 0, -180, 0, (0xa << 16), bhvSpinAirborneWarp),
		OBJECT(MODEL_EXCLAMATION_BOX, -14, -146, -16, 0, -180, 0, 0x00000000, bhvExclamationBox),
		OBJECT(MODEL_NONE, -4553, 188, -5480, 0, -180, 0, (0x60 << 24) | (0x80 << 16), bhvWarp),
		OBJECT(MODEL_NONE, -4553, 388, -5480, 0, -180, 0, (0x60 << 24) | (0x80 << 16), bhvWarp),
		OBJECT(MODEL_NONE, -4553, 589, -5480, 0, -180, 0, (0x60 << 24) | (0x80 << 16), bhvWarp),
		OBJECT(MODEL_NONE, -4553, 789, -5480, 0, -180, 0, (0x60 << 24) | (0x80 << 16), bhvWarp),
		OBJECT(MODEL_NONE, -4553, -413, -5480, 0, -180, 0, (0x60 << 24) | (0x80 << 16), bhvWarp),
		OBJECT(MODEL_NONE, -4553, -212, -5480, 0, -180, 0, (0x60 << 24) | (0x80 << 16), bhvWarp),
		OBJECT(MODEL_NONE, -4553, -12, -5480, 0, -180, 0, (0x60 << 24) | (0x80 << 16), bhvWarp),
		OBJECT(MODEL_NONE, -3195, -893, 3805, 0, 0, 0, (0x51 << 16) | (4 << 8), bhvBooCage),
		OBJECT(MODEL_NONE, -3288, -915, 1547, 0, 0, 0, (0x52 << 16) | (4 << 8), bhvBooCage),
		OBJECT(MODEL_NONE, -4889, -915, 2814, 0, 0, 0, (0x57 << 16) | (4 << 8), bhvBooCage),
		OBJECT(MODEL_NONE, -3695, -1074, 2801, 0, 0, 0, (0x56 << 16) | (4 << 8), bhvFadingWarp),
		OBJECT(MODEL_NONE, -3697, -1074, 2803, 0, 0, 0, (4 << 8), bhvSparkler),
		OBJECT(MODEL_LUIGI_NPC, -3698, -1015, 2605, 0, 89, 0, (4 << 8), bhvOWLuigi),
		OBJECT(MODEL_NONE, -14, -146, -16, 0, -180, 0, (0xf0 << 16), bhvAirborneStarCollectWarp),
		OBJECT(MODEL_NONE, 38, 2000, -6418, 0, -180, 0, (2 << 24), bhvStar),
		TERRAIN(castle_grounds_area_1_collision),
		MACRO_OBJECTS(castle_grounds_area_1_macro_objs),
		SET_BACKGROUND_MUSIC(0x00, SEQ_LEVEL_GRASS),
		TERRAIN_TYPE(TERRAIN_GRASS),
		/* Fast64 begin persistent block [area commands] */
		/*-1474, -1260, 4233*/
		OBJECT(MODEL_OW_ARTHUR , -2181, -477, -2762, 0, 0, 0, (2 << 16), bhvOWPart),
		OBJECT(MODEL_OW_BRODUTE, 3571, -229, 147, 0, 0, 0, (1 << 16), bhvOWPart),
		OBJECT(MODEL_OW_GAEL,    169, -1321, 3826, 0, 0, 0, (3 << 16), bhvOWPart),
		OBJECT(MODEL_OW_SIDE, -3702, -1194, 2710, 0, 0, 0, (4 << 16), bhvOWPart),
		OBJECT(MODEL_OW_SCUT, 4123, -102, 4749, 0, 0, 0, (5 << 16), bhvOWPart),
		OBJECT(MODEL_OW_BOWSER, -3754, 817, 1752, 0, 0, 0, (6 << 16), bhvOWPart),
		OBJECT(MODEL_OW_DNVIC_CLOSE, -1474, -1260, 4233, 0, 0, 0, (3 << 8), bhvOWDnvicClose),
		/* Fast64 end persistent block [area commands] */
	END_AREA(),

	FREE_LEVEL_POOL(),
	MARIO_POS(1, 180, 0, -500, 0),
	CALL(0, lvl_init_or_update),
	CALL_LOOP(1, lvl_init_or_update),
	CLEAR_LEVEL(),
	SLEEP_BEFORE_EXIT(1),
	EXIT(),
};
