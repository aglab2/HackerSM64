#include "src/game/envfx_snow.h"

const GeoLayout cotmc_area_1_geo[] = {
	GEO_NODE_START(),
	GEO_OPEN_NODE(),
		GEO_DISPLAY_LIST(LAYER_TRANSPARENT, cotmc_dl__bg_mesh_layer_5),
		GEO_DISPLAY_LIST(LAYER_OPAQUE_DECAL, cotmc_dl_s1_mesh_layer_2),
		GEO_DISPLAY_LIST(LAYER_OPAQUE, cotmc_dl_s1_mesh_layer_1),
		GEO_CULL(-20000, 20000, -20000, 20000, -20000, 6000),
		GEO_OPEN_NODE(),
			GEO_DISPLAY_LIST(LAYER_OPAQUE, cotmc_dl_s1_001_mesh_layer_1),
		GEO_CLOSE_NODE(),
		GEO_DISPLAY_LIST(LAYER_TRANSPARENT, cotmc_dl_s1_002_mesh_layer_5),
		GEO_DISPLAY_LIST(LAYER_OPAQUE, cotmc_dl_s1_003_mesh_layer_1),
		GEO_DISPLAY_LIST(LAYER_OPAQUE_INTER, cotmc_dl_s1_004_mesh_layer_3),
	GEO_CLOSE_NODE(),
	GEO_RETURN(),
};
const GeoLayout cotmc_area_1[] = {
	GEO_NODE_SCREEN_AREA(10, SCREEN_WIDTH/2, SCREEN_HEIGHT/2, SCREEN_WIDTH/2, SCREEN_HEIGHT/2),
	GEO_OPEN_NODE(),
		GEO_ZBUFFER(1),
		GEO_OPEN_NODE(),
			GEO_CAMERA_FRUSTUM_WITH_FUNC(45.0000, 100, 30000, geo_camera_fov),
			GEO_OPEN_NODE(),
				GEO_CAMERA(CAMERA_MODE_8_DIRECTIONS, 0, 0, 0, 0, -100, 0, geo_camera_main),
				GEO_OPEN_NODE(),
					GEO_BRANCH(1, cotmc_area_1_geo),
					GEO_RENDER_OBJ(),
					GEO_ASM(ENVFX_MODE_NONE, geo_envfx_main),
				GEO_CLOSE_NODE(),
			GEO_CLOSE_NODE(),
		GEO_CLOSE_NODE(),
		GEO_DISPLAY_LIST(LAYER_OPAQUE, cotmc_dl_material_revert_render_settings),
		GEO_DISPLAY_LIST(LAYER_OPAQUE_DECAL, cotmc_dl_material_revert_render_settings),
		GEO_DISPLAY_LIST(LAYER_OPAQUE_INTER, cotmc_dl_material_revert_render_settings),
		GEO_DISPLAY_LIST(LAYER_TRANSPARENT, cotmc_dl_material_revert_render_settings),
	GEO_CLOSE_NODE(),
	GEO_END(),
};
